...
<div id="data">
    ...
    <div class="centercontent tables" style="margin-left:10px;">
        ...
        <div id="contentwrapper" class="contentwrapper" style="padding:0px 20px 20px 20px">
            ...
            <!--BOTTONI PER UTENTE PRECEDENTE E SUCCESSIVO-->
            <div style="float:right;">

                <?php

                //Vado all'utente precedente
                $db->query(" SELECT id 
                                 FROM 
                                 bag_utenti WHERE id=(select max(id) from bag_utenti where id < '" . $_POST['idutente'] . "' ORDER BY bag_utenti.nome)
                                 ");
                $recordsi = $db->single();
                ...
                //             INNER JOIN province p ON p.id_regione = r.id
                //             INNER JOIN comuni c ON c.id_provincia = p.id
                //             WHERE c.id=" . $recordsi['id_comune'] . " ");
                // $listIndirizzoi = $db->single();
                ?>

                <a title="Utente Precedente" href="javascript:void(null)" onclick="jQuery('#data').load('<?= BASE_URL; ?>loaddettagliutente.php #data',{idutente:'<?= $recordsi['id']; ?>'},function(){}); ">
                    <img src="images/prev.png" />
                </a>

                <?php

                //Vado all'utente successivo
                $db->query(" SELECT id 
                                 FROM 
                                 bag_utenti WHERE id=(select min(id) from bag_utenti where id > '" . $_POST['idutente'] . "' ORDER BY bag_utenti.nome)
                                 ");
                $recordst = $db->single();
                ...
                //             INNER JOIN province p ON p.id_regione = r.id
                //             INNER JOIN comuni c ON c.id_provincia = p.id
                //             WHERE c.id=" . $recordst['id_comune'] . " ");
                // $listIndirizzost = $db->single();
                ?>

                <a href="javascript:void(null)" title="Utente Successivo" onclick="jQuery('#data').load('<?= BASE_URL; ?>loaddettagliutente.php #data',{idutente:'<?= $recordst['id']; ?>'},function(){}); ">
            ...
            <!-- FACCIO USCIRE I DATI SOLO L'ID UTENTE ESISTE -->
            <?php if ($_POST['idutente'] != '' or $_POST['idutente'] != 0) { ?>
