    jQuery(document).ready(function(){ 
       // jQuery('input:checkbox, input:radio, select.uniformselect, input:file').uniform();
        ...
        jQuery('body').on("blur",".invia",function() {
      
            var questo=jQuery(this);
            var idattuale=jQuery(this).attr("id");
            var idimg=jQuery(this).attr("for");
            
            jQuery.post("functionload.php", {table:'bag_prodotti',id:idattuale,valore:jQuery(this).val(),function:jQuery(this).attr("for")}, "json"  );
        ...
        jQuery('body').on("focus",".invia",function() {
           ...
           jQuery("#form_prodotti").submit(  
                ...
                function () { 
                         jQuery.post("functionload.php",jQuery("#form_prodotti").serialize(),
                              function(data) {  
                                          ...
                                          setTimeout(function(){window.location.href = "prodotti.php";},1500); 
...
jQuery("#scheda_t").fileinput({
        uploadUrl: './funzioni/uploadimg.php?folder=schede&id_file=scheda_t', // you must set a valid URL here else you will get an error
...
//    ],
          ...
          // console.log(data);
            jQuery('<input />').attr('type', 'hidden')
                               .attr('name', "scheda_tecnica")
                               .attr('value', data.response.nomefile)
                               .appendTo('#form_prodotti');
...
jQuery("#img_file").fileinput({
        uploadUrl: './funzioni/uploadimg.php?folder=prodotti&id_file=img_file', // you must set a valid URL here else you will get an error
        dropZoneEnabled:false,
        language: "it",
        maxFileSize: 16000,
        allowedFileExtensions:    ['jpg','jpeg','gif','png', 'txt','rtf','mp4','mp3','3gp','mov','xls','xlsx','doc','docx','pdf','bmp','jpeg','odt','ods','pptx','ppt','tiff'],
...
jQuery("#img_file_m").fileinput({
        uploadUrl: './funzioni/uploadimg.php?folder=prodotti&id_file=img_file_m', // you must set a valid URL here else you will get an error
...
//    ],
     ...
     jQuery('body').on("click", ".eliminafile", function () {

        var questo = jQuery(this);
        var idfile = questo.attr("data-id_file");

        var nuovodiagf = jQuery('<div style="font-size:14px;color:red;">Sei sicuro di voler eliminare questo file?</div>').dialog({
            resizable: true,
            modal: true,
//                    position: { my: "top",at:"top+10%" },
            position: {my: "right", at: "top+10%", of: "#div_elimina_file" + idfile},
            buttons: {
                "SI": function () {
                    jQuery.post("functionload.php", {id_file: idfile, function: 'elimina_file'}, function (data) {
                        jQuery('#div_file' + idfile).remove();
                    });

                    nuovodiagf.dialog("close");
                },
                "NO": function () {
                    nuovodiagf.dialog("close");
                }
