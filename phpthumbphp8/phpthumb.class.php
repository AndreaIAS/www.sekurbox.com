...
//		}
	...
	function ImageMagickWhichConvert() {
		...
		if (is_null($WhichConvert)) {
			if ($this->iswindows) {
				...
				$WhichConvert = trim((new phpthumb_functions) -> SafeExec('which convert'));
			}
		}
		return $WhichConvert;
	}
	...
	function ImageMagickVersion($returnRAW=false) {
		...
		if (is_null($versionstring)) {
			...
			if ($commandline) {
				...
				$versionstring[1] = trim((new phpthumb_functions)->SafeExec($commandline));
	...
	function ImageMagickSwitchAvailable($switchname) {
		...
		if (is_null($IMoptions)) {
			...
			if (!is_null($commandline)) {
				$commandline .= ' -help';
				$IMhelp_lines = explode("\n", (new phpthumb_functions)->SafeExec($commandline));
	...
	function ImageMagickFormatsList() {
		...
		if (is_null($IMformatsList)) {
			...
			if (!is_null($commandline)) {
				...
				$IMformatsList = (new phpthumb_functions)->SafeExec($commandline);
			}
		}
		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ImageMagickSwitchAvailable('thumbnail') && $this->config_imagemagick_use_thumbnail) {
					...
					$IMresult_test = (new phpthumb_functions)->SafeExec($commandline_test);
				...
				$IMresult = (new phpthumb_functions)->SafeExec($commandline);
				clearstatcache();
