...
if (!(new phpthumb_functions)->FunctionIsDisabled('set_time_limit')) {
...
if (@$_GET['src'] && isset($_GET['md5s']) && empty($_GET['md5s'])) {
	if (preg_match('/^(f|ht)tps?\:\/\//i', $_GET['src'])) {
		if ($rawImageData = (new phpthumb_functions)->SafeURLread($_GET['src'], $error, $phpThumb->config_http_fopen_timeout, $phpThumb->config_http_follow_redirect)) {
			$md5s = md5($rawImageData);
		}
	} else {
		$SourceFilename = $phpThumb->ResolveFilenameToAbsolute($_GET['src']);
		if (is_readable($SourceFilename)) {
			$md5s = (new phpthumb_functions)->md5_file_safe($SourceFilename);
		} else {
...
$parsed_url_referer = (new phpthumb_functions)->ParseURLbetter(@$_SERVER['HTTP_REFERER']);
...
$parsed_url_src = (new phpthumb_functions)->ParseURLbetter(@$_GET['src']);
...
//if (!@$_GET['phpThumbDebug'] && !is_file($phpThumb->sourceFilename) && !(new phpthumb_functions)->gd_version()) {
//	if (!headers_sent()) {
...
function SendSaveAsFileHeaderIfNeeded() {
	...
	global $phpThumb;
	$downloadfilename = (new phpthumb_functions)->SanitizeFilename(@$_GET['sia'] ? $_GET['sia'] : (@$_GET['down'] ? $_GET['down'] : 'phpThumb_generated_thumbnail'.(@$_GET['f'] ? $_GET['f'] : 'jpg')));
	if (@$downloadfilename) {
...
while ($CanPassThroughDirectly && $phpThumb->src) {
	...
	if ($phpThumb->getimagesizeinfo = @GetImageSize($SourceFilename)) {
		...
		if ($phpThumb->config_disable_onlycreateable_passthru || (function_exists($theImageCreateFunction) && ($dummyImage = @$theImageCreateFunction($SourceFilename)))) {
			...
			if ($contentType = (new phpthumb_functions)->ImageTypeToMIMEtype(@$phpThumb->getimagesizeinfo[2])) {
...
function RedirectToCachedFile() {
	...
	$parsed_url = (new phpthumb_functions)->ParseURLbetter(@$_SERVER['HTTP_REFERER']);
	...
	if ($phpThumb->config_nooffsitelink_enabled && @$_SERVER['HTTP_REFERER'] && !in_array(@$parsed_url['host'], $phpThumb->config_nooffsitelink_valid_domains)) {
		...
		if ($getimagesize = @GetImageSize($phpThumb->cache_filename)) {
			$phpThumb->DebugMessage('* Would have sent headers (2): Content-Type: '.(new phpthumb_functions)->ImageTypeToMIMEtype($getimagesize[2]), __FILE__, __LINE__);
		}
		...
		if ($getimagesize = @GetImageSize($phpThumb->cache_filename)) {
			header('Content-Type: '.(new phpthumb_functions)->ImageTypeToMIMEtype($getimagesize[2]));
...
if ($phpThumb->rawImageData) {
	...
	if (!(new phpthumb_functions)->IsHexColor($bghexcolor)) {
		$phpThumb->ErrorImage('BGcolor parameter for "new" is not valid');
	}
	$opacity = (strlen($opacity) ? $opacity : 100);
	if ($phpThumb->gdimg_source = (new phpthumb_functions)->ImageCreateFunction($phpThumb->w, $phpThumb->h)) {
		...
		$new_background_color = (new phpthumb_functions)->ImageHexColorAllocate($phpThumb->gdimg_source, $bghexcolor, false, $alpha);
	...
	$cleanedupurl = (new phpthumb_functions)->CleanUpURLencoding($phpThumb->src);
	$phpThumb->DebugMessage('CleanUpURLencoding('.$phpThumb->src.') returned "'.$cleanedupurl.'"', __FILE__, __LINE__);
	$phpThumb->src = $cleanedupurl;
	unset($cleanedupurl);
	if ($rawImageData = (new phpthumb_functions)->SafeURLread($phpThumb->src, $error, $phpThumb->config_http_fopen_timeout, $phpThumb->config_http_follow_redirect)) {
...
if ($phpThumb->config_allow_parameter_file && $phpThumb->file) {
	...
	(new phpthumb_functions)->EnsureDirectoryExists(dirname($phpThumb->cache_filename));
