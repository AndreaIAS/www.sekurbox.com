...
$db->query("SELECT * FROM bag_categorie
            WHERE bag_categorie.id='".$_REQUEST['id_cat']."'
         ");
$list = $db->single();
...
$categoria=$_REQUEST['id_cat'];  
    
$title_it=$title_en=ucfirst(strtolower($list['nome_'.$lng]))." | Sekurbox.com";
$description_it="Le migliori offerte di ".ucfirst(strtolower($list['nome_'.$lng]))." a prezzo conveniente e con consegna rapida";
$description_en="Best offer of ".ucfirst(strtolower($list['nome_'.$lng]))." at an affordable price and with fast delivery";
...
<!--                                        <button class="btn sorting-btn dropdown-toggle" data-path="default" type="button" data-toggle="dropdown">Default Sorting<span class="caret"></span>
                        ...
                        <div class="row inner-section-space-top include-item">
                            ...
                            $db->query("SELECT bag_prodotti.*,
                                        ...
                                        WHERE bag_prodotti.id_sottocategoria=bag_scat.id
                                        AND bag_prodotti.id_marca=bag_marche.id
                                        AND bag_scat.id_categoria='".$_REQUEST['id_cat']."'
                                        ORDER by posizione
                                       ");
                            $recordleft = $db->resultset();
                            foreach ($recordleft as $list) { 
